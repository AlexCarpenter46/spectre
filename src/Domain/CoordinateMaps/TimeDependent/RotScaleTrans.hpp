 ...
 * that the center of the map is at (0., 0., 0.). If translation or expansion is
 * specified, then an inner and outer radius must be specified as well. There is
 * a rigid flag which when set to true treats the coordinates as if they're
 * within the inner radius. This can be used as an optimization if you know
 * which blocks in the domain will always be within the inner radius.
 *
 ...
 * w_{T})\vec{T}(t), & R_{in} < |\vec{\xi}| \leq 0.9 R_{out}, \\ (w_{E} +
 * E_{b}(t))R(t)\vec{\xi} + w_{T}\vec{T}(t), & 0.9 R_{out} < |\vec{\xi}| <
 * R_{out}, \\ E_{b}(t)R(t)\vec{\xi}, & |\vec{\xi}| \geq R_{out}
 * \end{array}\right.
 * \f}
 *
 ...
 * - E_{b}(t))}{|\vec{\xi}|(R_{out} - R_{in})}, & R_{in} < |\vec{\xi}| \leq 0.9
 * R_{out}, \\ \frac{R_{in}(R_{out} - |\vec{\xi}|)(E_{a}(t) -
 * E_{b}(t))}{|\vec{\xi}|(R_{out} - R_{in})}, & 0.9 R_{out} < |\vec{\xi}| <
 ...
 * R_{in}}, & R_{in} < |\vec{\xi}| \leq 0.9 R_{out}, \\ \frac{R_{out} -
 * |\vec{\xi}|}{R_{out} - R_{in}}, & 0.9 R_{out} < |\vec{\xi}| < R_{out}
 * \end{array}\right.
 * \f}
 *
 ...
 * When closer to $R_{in}$ the quadratic has the form
 * \f{equation}{
 * w_{E}^2(\bar{\xi}^2(R_{out} - R_{in})^2 - \rho^2 R_{out}^2) +
 * 2w_{E}\rho(\bar{\xi}^2(R_{out} - R_{in}) - \rho R_{out}^2 E_{b}(t))
 ...
 * When closer to $R_{out}$ the quadratic has the form
 * \f{equation}{
 * w_{E}^2(\bar{\xi}^2(R_{out} - R_{in})^2 - \rho^2 R_{in}^2) +
 * 2w_{E}\rho(\bar{\xi}^2(R_{in} - R_{out}) - \rho R_{in}^2 E_{a}(t))
 ...
 * < 0.5 R_{out}, \\ R^{T}(t)(\vec{\bar{\xi}} - (1.0 - w_{T})\vec{T}(t)),
 * & 0.5 R_{out} \leq |\vec{\bar{\xi}}| < R_{out},
 ...
 * \f{equation}{
 * w_{T}^2(T(t)^2 - (R_{out} - R_{in})^2) -
 * 2w_{T}(\vec{T}(t) \cdot (\vec{T}(t) - \vec{\bar{\xi}}) + R_{in}(R_{out} -
 * R_{in})
 ...
 * dE_{b}(t)R(t))\vec{\xi}, & |\vec{\xi}| \geq R_{out} \end{array}\right.
 * \f}
 *
 ...
 * |\vec{\xi}|)(dE_{a}(t)
 * - dE_{b}(t))}{|\vec{\xi}|(R_{out} - R_{in})}, & R_{in} < |\vec{\xi}| \leq 0.9
 * R_{out}, \\ \frac{R_{in}(R_{out} - |\vec{\xi}|)(dE_{a}(t) -
 * dE_{b}(t))}{|\vec{\xi}|(R_{out} - R_{in})}, & 0.9 R_{out} < |\vec{\xi}| <
 * R_{out} \end{array}\right.
 * \f}
 *
 * ## Jacobian
 ...
 * |\vec{\xi}| \leq 0.9 R_{out}, \\ {R^{i}}_{j}(t)E_{b}(t) + \frac{\alpha
 * {R^{i}}_{l}(t)\vec{\xi}^{l}\vec{\xi}_{j}(E_{A}(t) - E_{B}(t))}{|\vec{\xi}|} +
 * w_{E}{R^{i}}_{j}(t) + \frac{dw_{T}T^{i}\xi_{j}}{|\vec{\xi}|}, & 0.9 R_{out}
 * < |\vec{\xi}| < R_{out}, \\ E_{b}(t){R^{i}}_{j}(t), & |\vec{\xi}|
 * \geq R_{out} \end{array}\right.
 * \f}
 *
