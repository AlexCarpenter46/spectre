...
namespace {
std::optional<std::array<double, 3>> extract_scalar_func_and_2_derivs(
    const domain::FunctionsOfTimeMap& functions_of_time,
    const std::string& function_of_time_name, const double time) {
  const auto function_of_time = functions_of_time.find(function_of_time_name);
  ...
  for (const auto& value : values) {
    if (value.size() != 1) {
      ERROR(function_of_time_name
            << " should have scalar values, but found a DataVector of size "
            << value.size() << ".");
    }
  }
...
std::optional<std::vector<std::array<double, 4>>>
extract_rotation_func_and_2_derivs(
  ...
  for (size_t deriv = 0; deriv < values.size(); ++deriv) {
    if (gsl::at(values, deriv).size() != 4) {
      ERROR(
          "Rotation should have quaternion values, but found a DataVector "
          "of size "
          << gsl::at(values, deriv).size() << ".");
    }
    auto& result_deriv = result[deriv];
    for (size_t component = 0; component < 4; ++component) {
      result_deriv[component] = gsl::at(values, deriv)[component];
    }
  }
  return result;
}

std::optional<std::array<std::array<double, 3>, 3>>
extract_translation_func_and_2_derivs(
    const domain::FunctionsOfTimeMap& functions_of_time, const double time) {
  ...
  for (size_t deriv = 0; deriv < values.size(); ++deriv) {
    if (gsl::at(values, deriv).size() != 3) {
      ERROR(
          "Translation should have 3 components, but found a DataVector "
          "of size "
          << gsl::at(values, deriv).size() << ".");
    }
    auto& result_deriv = result[deriv];
...
strahlkorper_coefs_and_centers(
  ...
  for (size_t i = 0; i < requested_number_of_times_from_end; ++i) {
    if (gsl::at(ahc_times, i) <= match_time) {
      ...
      const auto exp_outer_bdry_func_and_2_derivs_from_volume =
          extract_scalar_func_and_2_derivs(inspiral_functions_of_time,
                                           "ExpansionOuterBoundary",
                                           gsl::at(ahc_times, i));
      const auto rot_func_and_2_derivs_from_volume =
          extract_rotation_func_and_2_derivs(inspiral_functions_of_time,
                                             gsl::at(ahc_times, i));
      const auto trans_func_and_2_derivs_from_volume =
          extract_translation_func_and_2_derivs(inspiral_functions_of_time,
                                                gsl::at(ahc_times, i));
      ...
      ahc_ringdown_distorted_coefs.push_back(
          distorted_ahc.coefficients());

      tnsr::I<DataVector, 3, ::Frame::Grid> grid_center_point{
          DataVector{1, 0.0}};
      grid_center_point[0] =
          distorted_ahc.expansion_center()[0];
      grid_center_point[1] =
          distorted_ahc.expansion_center()[1];
      grid_center_point[2] =
          distorted_ahc.expansion_center()[2];
      tnsr::I<DataVector, 3, ::Frame::Inertial>
          inertial_center_point{DataVector{1, 0.0}};
      ...
      coords_to_different_frame(
          make_not_null(&inertial_center_point),
          grid_center_point, temporary_ringdown_domain_non_replay,
          ringdown_functions_of_time_non_replay, gsl::at(ahc_times, i));
  ...
  return std::pair{ahc_ringdown_distorted_coefs,
