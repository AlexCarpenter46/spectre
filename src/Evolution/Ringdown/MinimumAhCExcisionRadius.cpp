...
double minimum_ahc_excision_radius(
  ...
  const std::array<double, 3> ahc_center =
  ...
  const double q = abs(excision_B_center[0] / excision_A_center[0]);
  const double eps = 1e-3 / q;
    ...
    while (not inner_converged and current_inner_iteration < max_iterations) {
      ...
      for (size_t s = 0; s < get<0>(excision_a_inspiral_inertial_points).size();
        ...
        const double excision_a_point_radius =
            sqrt(square(get<0>(exc_a_ringdown_grid_point) - ahc_center[0]) +
                 square(get<1>(exc_a_ringdown_grid_point) - ahc_center[1]) +
                 square(get<2>(exc_a_ringdown_grid_point) - ahc_center[2]));
        const double excision_b_point_radius =
            sqrt(square(get<0>(exc_b_ringdown_grid_point) - ahc_center[0]) +
                 square(get<1>(exc_b_ringdown_grid_point) - ahc_center[1]) +
                 square(get<2>(exc_b_ringdown_grid_point) - ahc_center[2]));
        min_excision_radius =
      ...
      // between the minimum possible value to fit excisions A/B and the common
