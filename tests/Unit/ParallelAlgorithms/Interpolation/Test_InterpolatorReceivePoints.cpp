...
SPECTRE_TEST_CASE("Unit.NumericalAlgorithms.Interpolator.ReceivePoints",
  ...
  // Send points to both interpolator cores. Not much should happen
  for (size_t array_index = 0; array_index < 2; array_index++) {
  ...
  // not have done an interpolation to these new points because of iteration 0
  runner.simple_action<
      interp_component,
      intrp::Actions::InterpolatorReceiveVolumeData<Tags::TimeStepId>>(
  ...
  // Now send with iteration 1. These points should overwrite the previous ones
